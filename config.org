...
Simple protein diagram program using svg.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
(defun de/pcr-format-time (seconds)
  "Converts seconds into a MM:SS string."
  (let* ((sec (ceiling seconds))
         (m (/ sec 60))
         (s (% sec 60)))
    (format "%d:%02d" m s)))

(defun org-dblock-write:pcr-protocol (params)
  "Generates a PCR protocol based on PARAMS.
PARAMS should include :polymerase, :length-bp, and :tm."
  (let* ((polymerase (plist-get params :polymerase))
         (length-bp  (plist-get params :length-bp))
         (ncycles    (plist-get params :ncycles))
         (tm         (plist-get params :tm))

         ;; polymerase type
         (is-taq     (string-match-p "taq" polymerase))
         (is-hifi    (or (string-match-p "q5" polymerase)
                         (string-match-p "phusion" polymerase)))
         (is-q5      (string-match-p "q5" polymerase))

         ;; figure out temperatures
         ;; taq uses 95 for denaturation and 68 for extension
         (denat-temp (if is-taq 95 98))
         (ext-temp   (if is-taq 68 72))
         
         ;; Calculate extension time
         (length-kb  (/ length-bp 1000.0))
         (seconds-per-kb (if is-taq 60 30)) ;; taq uses 60s/kb, others use 10-30s/kb
         (raw-ext-seconds (max 10 (* length-kb seconds-per-kb))) ;; use at least 10 secs
         (ext-time-str (de/pcr-format-time raw-ext-seconds))

         ;; annealing logic
         (anneal-temp (cond (is-hifi (+ tm 3))
                            (is-taq  (- tm 5))
                            (t tm)))
         (anneal-note (cond (is-hifi "Tm + 3 °C")
                            (is-taq "Tm - 5 °C")
                            (t "Tm")))

         ;; Final extension defaults
         (final-ext-time (if is-q5 "2:00" "5:00")))

    (insert
     (format "| Step | Temp. | Time | Notes |\n")
     (format "|------|-------|------|-------|\n")
     (format "| init. denaturation | %d °C | 0:30 | |\n" denat-temp)
     (format "| %d cycles | | | |\n" ncycles)
     (format "| - denaturation | %d °C | %s | |\n"
             denat-temp (if is-taq "0:30" "0:10"))
     (format "| - annealing    | %d °C | %s | Calc: %s |\n"
             anneal-temp (if is-taq "1:00" "0:30") anneal-note)
     (format "| - extension    | %d °C | %s | %.2f kb * %ds/kb |\n"
             ext-temp ext-time-str length-kb seconds-per-kb)
     (format "|------|-------|------|-------|\n")
     (format "| Final extension| %d °C | %s | |\n" ext-temp final-ext-time)
     (format "| hold | 12 °C | \u221e | |"))
    (org-table-align)))


(defun de/insert-pcr-template ()
  "Interactively insert a PCR dynamic block."
  (interactive)
  (let ((poly (completing-read "Polymerase: " '("q5" "phusion" "taq") nil t "q5"))
        (len (read-number "Amplicon Length (bp): "))
        (tm (read-number "Primer Tm (°C): "))
        (ncycles (read-number "# cycles: ")))
    (insert
     (format "#+BEGIN: pcr-protocol :polymerase \"%s\" :length-bp %d :tm %d :ncycles %d\n#+END\n"
             poly len tm ncycles))
    (forward-line -1)
    (org-ctrl-c-ctrl-c)))
#+end_src

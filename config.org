...
With claude.ai, it has become much easier to write both backend (using rust) and front-end (using TypeScript/Javascript) for a GUI application with tauri. 
...
(custom-set-faces
 '(nano-mono ((t (:family "Fragment Mono"
			  :height 120
			  :weight regular)))))

...
I find myself using more of ~artist-mode~ to write plain ASCII notes because I
prefer to not do any context switching when working on a problem or learning
new concepts. I'll define a personal prefix map for entering and exiting this
mode so that it would not interfere with commonly used keybindings like =C-c
C-c=. Use =C-c u a= to enter artist-mode and =C-c u a= again to exit.
...
To justify both sides evenly like Rougier's examples. Use enriched-mode and
press 'M-j b' on each paragraph. This runs ~set-justification-full~ on the
current paragraph. *But be careful!* This actually changes the document as
rich-text and adds markup (tags) to the text file.
...
;; use Rougier's nano emacs
(use-package nano
  :straight (:type git :host github :repo "rougier/nano-emacs")
  ;; don't load nano.el, load each modules explicitly
  :no-require t 
  :init
  ;; Optional: for variable-pitch
  ...
  :config
  ;; workaround, somehow NANO needs this face
  ...
  (require 'nano-layout)
  ;; this also sets `frame-background-mode` needed for functions below
  (nano-theme-set-light) ;; set default to 'light'

  ;; define custom fonts
  (defun de/nano-font-stack ()
    (set-face-attribute 'default nil
                        :family "Roboto Mono" :weight 'light :height 140)
    (set-face-attribute 'bold nil
                        :family "Roboto Mono" :weight 'regular)
    (set-face-attribute 'italic nil
                        :family "Victor Mono" :weight 'semibold :slant
                        'italic)
    (set-fontset-font t 'unicode
                      (font-spec :name "Fira Code" :size 14) nil)
    ;; set the range of nerd fonts xe000 to xf8ff
    (set-fontset-font t '(#xe000 . #xf8ff)
                      (font-spec :name "RobotoMono Nerd Font" :size 12)
                      nil))

  ;; define custom org source block colors
  (defun de/nano-org-block-faces ()
    "Apply custom org-block faces for nano theme."
    (let ((block-bg (if (eq frame-background-mode 'light)
                        "#F5F5F5"    ; slightly darker than white
                      "#3B4252")))   ; slightly lighter than nord dark
  ...
  ;; derive corfu faces from nano
  (defun de/nano-corfu-faces ()
    "Derive corfu popup faces from nano faces."
    (with-eval-after-load 'corfu
      (set-face 'corfu-default 'nano-face-subtle)
      (set-face 'corfu-current '(nano-face-strong nano-face-subtle))
      (set-face-attribute 'corfu-current nil :background nano-color-highlight)
      (set-face 'corfu-bar 'nano-face-faded)
      (set-face-attribute 'corfu-bar nil :background nano-color-foreground)
      (set-face 'corfu-border 'nano-face-subtle)
      (set-face-attribute 'corfu-border nil :background nano-color-faded)
      (set-face 'corfu-annotations 'nano-face-faded)
      (set-face 'corfu-deprecated 'nano-face-faded)))

  ;; force corfu to recreate popup after theme change
  (defun de/corfu-reset-frames (&rest _)
    "Kill corfu child frames so they're recreated with new colors."
    (dolist (frame (frame-list))
      (when (frame-parameter frame 'corfu--frame)
        (delete-frame frame))))
  
  ;; setup hooks and advice
  (advice-add 'nano-theme :after #'de/nano-font-stack)
  (advice-add 'nano-theme :after #'de/nano-org-block-faces)
  (advice-add 'nano-theme :after #'de/nano-corfu-faces)
  (advice-add 'nano-toggle-theme :after #'de/nano-org-block-faces)
  (advice-add 'nano-refresh-theme :after #'de/corfu-reset-frames)

  ;; Reapply when org loads
  (with-eval-after-load 'org
    (de/nano-org-block-faces))
  ...
  ;; initialize theme
  (nano-faces)
  (nano-theme) ;; all advice should be triggered correctly  
  
  ;; undo 'meta' key reassignment. I'm already used to 'alt' as 'meta'
...
;; claude-modified Rougier's package
(use-package svg-lib
  :straight nil
  :load-path "~/Apps/emacs/de-svg-lib")

(use-package svg-lib-org
  :straight nil
  :load-path "~/Apps/emacs/de-svg-lib"
  :after (svg-lib org)
  :commands svg-lib-org-mode
  :hook (org-mode . svg-lib-org-mode))
#+end_src
...
(use-package org
  ...
  ;; make sure org-indent is there
  (require 'org-indent nil t)
  ;; fix org indent issue for multi-level bullets
